  ...
  return withManagedOperationLock(RUNTIME_INSTALL_LOCK_KEY, async () => {
    ...
    if (isWin) {
      ...
      if (needNode && nodeInstallerPath) {
        ...
        if (capabilityError) return capabilityError
        // 使用 msiexec 提权静默安装 Node.js
        const psScript = `$ErrorActionPreference = 'Stop'; $p = Start-Process -FilePath "msiexec.exe" -ArgumentList "/i \`"$env:QCLAW_NODE_MSI_PATH\`" /qn /norestart" -Verb RunAs -Wait -PassThru; exit $p.ExitCode`
        const nodeResult = await runShell(
          ...
          {
            controlDomain: 'env-setup',
            env: { QCLAW_NODE_MSI_PATH: nodeInstallerPath },
          }
        )
        if (!nodeResult.ok) {
          return nodeResult
...
async function runWindowsElevatedGatewayCommand(action: string): Promise<CliResult> {
  const binaryPath = await resolveOpenClawBinaryPath()
  if (!binaryPath) return { ok: false, stdout: '', stderr: 'OpenClaw executable not found for elevation', code: -1 }

  // Do not hide the window, so the user can see execution status. Check ERRORLEVEL and pause if failed.
  const psScript = `$ErrorActionPreference = 'Stop'; $p = Start-Process -FilePath "cmd.exe" -ArgumentList "/c \`"${binaryPath}\`" gateway ${action} & if ERRORLEVEL 1 pause" -Verb RunAs -Wait -PassThru; exit $p.ExitCode`
  return runShell(
    'powershell.exe',
    ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', psScript],
    MAIN_RUNTIME_POLICY.cli.defaultCommandTimeoutMs + 30000,
    'gateway'
  )
}

export async function gatewayStart(): Promise<CliResult> {
  const r = await runCli(['gateway', 'start'], MAIN_RUNTIME_POLICY.cli.defaultCommandTimeoutMs, 'gateway')
  if (!r.ok && process.platform === 'win32' && r.stderr.toLowerCase().includes('schtasks')) {
    return runWindowsElevatedGatewayCommand('start')
  }
  return r
}

export async function gatewayInstall(): Promise<CliResult> {
  const r = await runCli(['gateway', 'install'], MAIN_RUNTIME_POLICY.cli.defaultCommandTimeoutMs, 'gateway')
  if (!r.ok && process.platform === 'win32' && r.stderr.toLowerCase().includes('schtasks')) {
    return runWindowsElevatedGatewayCommand('install')
  }
  return r
}
...
async function gatewayRestartImpl(): Promise<CliResult> {
  const r = await runCli(['gateway', 'restart'], MAIN_RUNTIME_POLICY.cli.defaultCommandTimeoutMs, 'gateway')
  if (!r.ok && process.platform === 'win32' && r.stderr.toLowerCase().includes('schtasks')) {
    return runWindowsElevatedGatewayCommand('restart')
  }
  return r
}
...
export async function gatewayForceRestart(): Promise<CliResult> {
  const r = await runCli(['gateway', 'restart'])
  if (!r.ok && process.platform === 'win32' && r.stderr.toLowerCase().includes('schtasks')) {
    return runWindowsElevatedGatewayCommand('restart')
  }
  return r
...
export async function gatewayStop(): Promise<CliResult> {
  const r = await runCli(['gateway', 'stop'], MAIN_RUNTIME_POLICY.cli.gatewayStopTimeoutMs, 'gateway')
  if (!r.ok && process.platform === 'win32' && r.stderr.toLowerCase().includes('schtasks')) {
    return runWindowsElevatedGatewayCommand('stop')
  }
  return r
}
