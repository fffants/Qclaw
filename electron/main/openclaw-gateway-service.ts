...
const GATEWAY_SERVICE_NOT_LOADED_PATTERN = /(?:\bgateway service not loaded\b)|(?:系统找不到指定的文件)|(?:openclaw gateway install)/i
...
async function ensureGatewayRunningImpl(
  ...
  if (isGatewayServiceNotLoaded(startResult)) {
    ...
    const installGatewayResult = await gatewayInstall()
