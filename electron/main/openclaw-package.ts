...
async function resolvePackageLayout(
  ...
  let packageLocation = await findNearestOpenClawPackageLocation(path.dirname(resolvedBinaryPath), fsPromises)

  if (!packageLocation && process.platform === 'win32') {
    // Windows cmd-shim fallback: npm global installs on Windows use .cmd wrappers instead of symlinks.
    // The actual module resides in `node_modules/{pkg}` relative to the wrapper directory.
    const cmdShimFallbackDir = path.join(path.dirname(resolvedBinaryPath), 'node_modules', 'openclaw')
    packageLocation = await findNearestOpenClawPackageLocation(cmdShimFallbackDir, fsPromises)
  }
